...
ENDCLASS.                    "lcl_alv_event_dynamic DEFINITION
...
ENDCLASS.                    "lcl_alv_factory DEFINITION
...
ENDCLASS.                    "lcl_config_manager DEFINITION
...
ENDCLASS.                    "lcl_alv_utils DEFINITION
...
CLASS lcl_alv_event_dynamic IMPLEMENTATION.
  ...
  METHOD handle_toolbar.
    ...
    FIELD-SYMBOLS <cfg> LIKE LINE OF gt_alv_config.
    READ TABLE gt_alv_config WITH KEY dynnr = sy-dynnr ASSIGNING <cfg>.
...
ENDCLASS.                    "lcl_alv_event_dynamic IMPLEMENTATION
...
CLASS lcl_alv_factory IMPLEMENTATION.
  ...
  METHOD create_alv.
    ...
    ls_config = lcl_config_manager=>get_alv_config( iv_dynnr = iv_dynnr ).
    ...
    IF sy-subrc NE 0.
      MESSAGE e001(00) WITH 'Container error' sy-subrc.
    ENDIF.

    FIELD-SYMBOLS <cfg> LIKE LINE OF gt_alv_config.
    READ TABLE gt_alv_config WITH KEY dynnr = iv_dynnr ASSIGNING <cfg>.
    IF sy-subrc EQ 0.
  ...
  METHOD apply_field_config.
    DATA ls_fc LIKE LINE OF gt_field_config.
    ...
    LOOP AT ct_fcat ASSIGNING <ls_fcat>.
      ...
      CLEAR ls_fc.
      READ TABLE gt_field_config INTO ls_fc
  ...
  METHOD setup_layout.
    ...
    ASSIGN COMPONENT 'C_COL' OF STRUCTURE <ls_row> TO <color>.
    IF sy-subrc EQ 0. rs_layout-ctab_fname  = 'C_COL'. ENDIF.

    FIELD-SYMBOLS <style> TYPE any.
    ASSIGN COMPONENT 'C_STY' OF STRUCTURE <ls_row> TO <style>.
    IF sy-subrc EQ 0. rs_layout-stylefname = 'C_STY'. ENDIF.

  ENDMETHOD.                    "setup_layout
...
ENDCLASS.                    "lcl_alv_factory IMPLEMENTATION
...
CLASS lcl_config_manager IMPLEMENTATION.
  ...
  METHOD register_alv.
    FIELD-SYMBOLS <alv_config> LIKE LINE OF gt_alv_config.
    APPEND INITIAL LINE TO gt_alv_config ASSIGNING <alv_config>.
    <alv_config>-dynnr          = iv_dynnr    .
    <alv_config>-container_name = iv_container.
    <alv_config>-structure_name = iv_structure.
    <alv_config>-title_key      = iv_title    .
    <alv_config>-pf_status      = iv_pf_status.
  ENDMETHOD.                    "register_alv

  METHOD add_field_config.
    APPEND is_field_config TO gt_field_config.
  ENDMETHOD.                    "add_field_config
  ...
  ENDMETHOD.                    "add_toolbar_button

  METHOD get_alv_config.
    READ TABLE gt_alv_config INTO rs_config WITH KEY dynnr = iv_dynnr.
  ENDMETHOD.                    "get_alv_config

  METHOD get_alv_ref.
    FIELD-SYMBOLS <cfg> LIKE LINE OF gt_alv_config.
    READ TABLE gt_alv_config WITH KEY dynnr = iv_dynnr ASSIGNING <cfg>.
    CHECK sy-subrc EQ 0.
    ro_alv = <cfg>-alv_grid_ref.
  ENDMETHOD.                    "get_alv_ref

  METHOD has_unsaved_changes.
    FIELD-SYMBOLS <cfg> LIKE LINE OF gt_alv_config.
    READ TABLE gt_alv_config WITH KEY dynnr = iv_dynnr ASSIGNING <cfg>.
    CHECK sy-subrc EQ 0.
    IF <cfg>-changed_data  IS NOT INITIAL
      OR <cfg>-deleted_data  IS NOT INITIAL
      OR <cfg>-inserted_data IS NOT INITIAL.
      rv_dirty = abap_true.
    ENDIF.
  ENDMETHOD.                    "has_unsaved_changes
  ...
  ENDMETHOD.                    "clear_changed_data

  METHOD set_screen_properties.
    DATA ls_config TYPE ts_alv_config.
    ls_config = lcl_config_manager=>get_alv_config( sy-dynnr ).
    ...
    ENDIF.
  ENDMETHOD.                    "set_screen_properties

ENDCLASS.                    "lcl_config_manager IMPLEMENTATION
...
CLASS lcl_alv_utils IMPLEMENTATION.
  ...
  METHOD confirm_unsaved_changes.
    ...
    IF lv_answer EQ '1'.
      rv_proceed = abap_true.
    ELSE.
      rv_proceed = abap_false.
    ENDIF.
  ENDMETHOD.                    "confirm_unsaved_changes
  ...
  METHOD apply_changed_data.
    ...
    FIELD-SYMBOLS: <chng> LIKE LINE OF ls_config-changed_data,
                   <row>  TYPE any,
                   <val>  TYPE any.
    LOOP AT ls_config-changed_data ASSIGNING <chng>.
      READ TABLE ct_table INDEX <chng>-row_id ASSIGNING <row>.
      CHECK sy-subrc EQ 0.
      ASSIGN COMPONENT <chng>-fieldname OF STRUCTURE <row> TO <val>.
      CHECK sy-subrc EQ 0.
      <val> = <chng>-value.
    ENDLOOP.
  ENDMETHOD.                    "apply_changed_data
  ...
  METHOD get_fieldcat_from_data.
    ...
    TRY.
        ...
        cl_salv_table=>factory(
          IMPORTING r_salv_table = lt_salv_table
...
ENDCLASS.                    "lcl_alv_utils IMPLEMENTATION
...
ENDFORM.                    "initialize_dynamic_alv
...
FORM call_alv_screen USING iv_dynnr    TYPE sy-dynnr
  ...
  READ TABLE gt_alv_config WITH KEY dynnr = iv_dynnr ASSIGNING <cfg>.
  IF sy-subrc EQ 0.
    <cfg>-data_table_ref = ir_data_ref.
  ENDIF.
